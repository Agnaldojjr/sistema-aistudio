...
export class TreatmentPlanning3DService {
  /**
   * Simula a busca de um plano de tratamento ativo para o paciente
   */
  static async getActivePlan(patientId: string): Promise<TreatmentPlan> {
   ...
   */
  static async getPlanTeeth(planId: string): Promise<TreatmentTooth[]> {
    return new Promise((resolve) => {
      setTimeout(() => {
   ...
   */
  static async getPlanProcedures(toothIds: string[]): Promise<TreatmentProcedure[]> {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve([]);
      }, 300);
